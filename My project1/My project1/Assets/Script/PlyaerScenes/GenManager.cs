...
public class GenManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            Ray cast = Camera.main.ScreenPointToRay(Input.mousePosition);       //2D 화면에서 유니티의 3D 화면으로 전환 하는 함수 ray 쓸때
            ...
            if(Physics.Raycast(cast, out hit))      //Ray를 쐈을 때 물리 결과가 일어나면
            {
                ...
                {
                    GameObject temp = (GameObject)Instantiate(Monster);         //프리팹 생성 함수(몬스터를 생서)
